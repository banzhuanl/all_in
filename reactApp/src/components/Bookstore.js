...
function BookTags (props) {
  ...
  return (
    <ul className="b-tags">
      {
        props.tags.map(item => (
          <li key={item} className={item===type?'active':''} onClick={() => props.checkType(item)}>{item}</li>
        ))
      }
    </ul>
  );
...
class Bookstore extends Component {
  ...
  getSlider(){
    Api.get(`http://127.0.0.1:3001/carousel`)
    .then(res => {
      this.setState({
        data: res.data || []
      });
  ...
  getBookList (id) {
    // 请求阅读喜好类型的数据
    Api.get(`http://127.0.0.1:3001/book/${id}`)
    .then(res => {
      this.setState({
  ...
  render() {
    ...
    const bookData = type === 'all'?this.state.bookData:this.state.bookData.filter( item => {
    ...
    return (
      <div className="Bookstore">
        ...
        <BookListWrap 
          title="精品推荐" 
          children={
            [<BookTags key="0" tags={this.state.tags} type={this.state.type} checkType={this.checkType.bind(this)}/>,
            <InfoItemList key="1" data={bookData}/>]
          }
        />
      </div>
    );
